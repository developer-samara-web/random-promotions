...
export async function GET(request, ctx) {
    try {
        // Получаем id раздачи
        const { promotion_id } = await ctx?.params;
        // Проверяем id раздачи
        if (promotion_id === 'undefined') { return NextResponse.json({ status: 404, error: 'Ошибка при загрузке участий.' }) }
        // Подключаемся к базе данных
        await connectToDatabase();
        // Получаем участия
        const participant = await Participant.find({ promotion_id, status: 'winner' }).populate(
